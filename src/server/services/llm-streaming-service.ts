...
export class LLMStreamingService {
  ...
  createStream(
    ...
    let isAborted = false;
    let fullContent = '';
    const toolCallsData: Array<{ name: string; args: Record<string, unknown>; result: string; confidence: number }> = [];
        ...
        try {
          ...
          if (toolExecutions.length > 0) {
            for (const te of toolExecutions) {
              toolCallsData.push({ name: te.name, args: te.args, result: te.result, confidence: te.confidence });
...
// Add tool call results to sources for citation panel display
for (const te of toolCallsData) {
  const toolType = getToolSourceType(te.name);
  
  // Determine stable ID based on tool type
  let knowledgeItemId: string | undefined;
  if (toolType === 'product') {
    knowledgeItemId = (te.args.sku ?? te.args.product_id) as string | undefined;
  } else if (toolType === 'size_chart') {
    knowledgeItemId = (te.args.size_chart_id ?? te.args.sku) as string | undefined;
  }
  
  // Try to parse image_url from result
  let imageUrl: string | null = null;
  try {
    const parsed = JSON.parse(te.result);
    imageUrl = parsed.image_url ?? parsed.imageUrl ?? null;
  } catch {
    // Not JSON or missing image_url
  }
  
  sources.push({
    type: toolType,
    name: te.name,
    content: te.result.substring(0, 500),
    score: te.confidence,
    provenanceVersion: 2,
...
if (claimVerificationResult !== null) {
  ...
  // Full success: no change needed (all knowledge citations remain)
          ...
          // Skip if the client disconnected before we reached this point
          if (options.abortSignal?.aborted) {
            logger.agent.debug('[LLMStreamingService] Skipping post-stream ops — client disconnected', { conversationId });
          } else {
            handlePostStreamOperations(
              conversationId,
              userMessage,
  ...
  private async handlePostStreamOperations(
    ...
    toolCallsData: Array<{ name: string; args: Record<string, unknown>; result: string; confidence: number }>,
    ...
    // P0-C: Skip all DB work if the client disconnected before reaching this point
    if (abortSignal?.aborted) {
      logger.agent.debug('[LLMStreamingService] Skipping post-stream ops due to client disconnect', { conversationId });
      return;
    }
    try {
...
/**
 * Map tool name to source type for citation panel.
 */
function getToolSourceType(toolName: string): string {
  switch (toolName) {
    case 'query_product_detail':
      return 'product';
    case 'query_size_chart':
      return 'size_chart';
    case 'query_order':
      return 'order';
