    ...
    try {
      if (isDemoMode()) {
        return new Response(JSON.stringify({
          ...
          useRerank: false,
        }), { status: 200, headers: { 'Content-Type': 'application/json', [REQUEST_ID_HEADER]: requestId } });
      }
...
export const PUT = withApi(
  ...
  async ({ request }) => {
    ...
    try {
      ...
      if (isDemoMode()) {
        return new Response(JSON.stringify({ success: true, demo: true }), { status: 200, headers: { 'Content-Type': 'application/json', [REQUEST_ID_HEADER]: requestId } });
      }
      ...
      return new Response(JSON.stringify({ success: true }), { status: 200, headers: { 'Content-Type': 'application/json', [REQUEST_ID_HEADER]: requestId } });
    } catch (err) {
