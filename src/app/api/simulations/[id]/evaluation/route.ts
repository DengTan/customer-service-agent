...
import { GET as defineGet, POST as definePost, PATCH as definePatch, DELETE as defineDelete } from '@/lib/api/with-api';
...
export const GET = defineGet(
  ...
  async ({ request, user, params }) => {
    const { id: simulationId } = params as { id: string };
    ...
    if (!canAccessSimulation(simulation.created_by, userId ?? null, role ?? null)) {
...
export const POST = definePost(
  ...
  async ({ request, user, params }) => {
    ...
    if (!canAccessSimulation(simulation.created_by, userId, role ?? null)) {
