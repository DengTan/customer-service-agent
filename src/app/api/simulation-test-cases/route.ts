...
import { apiSuccess, apiError, HttpStatus, getAuthenticatedUserId, parseJsonBody } from '@/lib/api-utils';
import { simulationTestCaseRepository, type TestCaseStatus } from '@/server/repositories/simulation-test-case-repository';
import { logger } from '@/lib/logger';
import { GET as defineGet, POST as definePost, PUT as definePut, DELETE as defineDelete } from '@/lib/api/with-api';
...
export const GET = defineGet(
  ...
  async ({ request }) => {
    ...
    const status = url.searchParams.get('status') as TestCaseStatus | undefined;
