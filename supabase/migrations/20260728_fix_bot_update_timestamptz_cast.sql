-- Fix: Cast p_expected_updated_at TEXT to timestamptz for comparison with updated_at
-- Error: "operator does not exist: timestamp with time zone = text"
-- The WHERE clause on line 65 of upsert_bot_config compares updated_at (timestamptz)
-- against p_expected_updated_at (TEXT), which PostgreSQL cannot do implicitly.
...
BEGIN
  ...
  UPDATE bot_configs SET
    ...
    parent_bot_id           = COALESCE(p_parent_bot_id, parent_bot_id),
    ...
    updated_at             = NOW()
  WHERE id = p_id
    AND (p_expected_updated_at IS NULL OR updated_at = p_expected_updated_at::timestamptz)
...
-- Re-grant execute (required after CREATE OR REPLACE)
